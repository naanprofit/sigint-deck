...
# SIGINT-Deck Raspberry Pi Installation Script
# Tested on: Raspberry Pi Zero 2 W, Pi 4, Pi 5 (Debian Bookworm ARM64)
# Usage: curl -sSL https://raw.githubusercontent.com/naanprofit/sigint-deck/main/scripts/install-pi.sh | bash
...
echo -e "${CYAN}======================================${NC}"
echo -e "${CYAN} SIGINT-Deck Pi Installer${NC}"
echo -e "${CYAN}======================================${NC}"
echo ""
...
# ============================================
echo -e "${YELLOW}[6/8] Setting up SIGINT-Deck...${NC}"
mkdir -p "$INSTALL_DIR"
...
if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    git clone https://github.com/naanprofit/sigint-deck.git "$INSTALL_DIR" 2>/dev/null || true
fi
...
sudo tee /etc/systemd/system/sigint-deck.service > /dev/null << EOF
[Unit]
...
echo "  2. Place it at $INSTALL_DIR/sigint-deck"
echo "  3. Set capabilities: sudo setcap cap_net_raw,cap_net_admin+eip $INSTALL_DIR/sigint-deck"
echo "  4. Connect USB WiFi adapter to wlan1"
echo "  5. Start: sudo systemctl start sigint-deck"
echo ""
