...
# SIGINT-Deck Raspberry Pi Uninstall Script
...
echo -e "${CYAN}======================================${NC}"
echo -e "${CYAN} SIGINT-Deck Pi Uninstaller${NC}"
echo -e "${CYAN}======================================${NC}"
echo ""
...
if systemctl is-active --quiet sigint-deck.service 2>/dev/null; then
    sudo systemctl stop sigint-deck.service
    echo -e "${GREEN}  Stopped sigint-deck.service${NC}"
fi
if systemctl is-enabled --quiet sigint-deck.service 2>/dev/null; then
    sudo systemctl disable sigint-deck.service
    echo -e "${GREEN}  Disabled sigint-deck.service${NC}"
fi
sudo rm -f /etc/systemd/system/sigint-deck.service
...
echo ""
echo -e "To reinstall: ${CYAN}curl -sSL https://raw.githubusercontent.com/naanprofit/sigint-deck/main/scripts/install-pi.sh | bash${NC}"
echo ""
